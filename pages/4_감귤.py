...
tab1, tab2 = st.tabs(['감귤 출하량 예측', '감귤 모델 재학습'])

with tab1:
    st.header('감귤 출하량 예측')
    data = pd.read_excel('pages/tangerines_train_data.xlsx', sheet_name=0)
    st.write("현재 감귤 데이터 확인", data)
    ...
    with st.form('Area-form'):
        ...
        if submitted:
            ...
            with st.spinner('모델 로딩 중입니다.'):
                m_neural_prophet = load("pages/tangerines_neuralprophet_model.np") #모델 로드
                time.sleep(5)
            st.success('예측 완료!')
            days = 365
            ...
            np_data = np.array(forecast_value)
            week_weight = pd.read_excel('pages/적용할_감귤_요일가중치.xlsx', sheet_name=0)
            ...
            model_scores = pd.read_excel('pages/tangerines_neuralprophet_model_MAE_RMSE.xlsx', sheet_name=0)
...
with tab2:
    ...
    old_data = pd.read_excel('pages/tangerines_train_data.xlsx', sheet_name=0)
    ...
    with st.form('학습데이터 업로드'):
        ...
        if submitted: 
                ...
                with st.spinner('Neural Prophet 학습중입니다.'):
                    ...
                    set_random_seed(0)
                    col_lst = ['money', 'temp', 'max_Temp', 'min_Temp', 'hum', 'wind', 'rain', 'sun_Qy', 'condens_Time', 'gr_Temp', 'soil_Temp', 'sun_day'] 
                    ...
                    df.to_excel('pages/tangerines_neuralprophet_model_MAE_RMSE.xlsx', index=False) # 파일로 저장
                    ...
                    save(real_m_neuralprophet, "pages/tangerines_neuralprophet_model.np")
                    time.sleep(5)
