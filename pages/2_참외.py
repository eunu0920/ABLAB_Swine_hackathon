...
tab1, tab2 = st.tabs(['참외 출하량 예측', '참외 모델 재학습'])
with tab1:
    st.header('참외 출하량 예측')
    data = pd.read_excel('pages/smallmelon_train_data.xlsx', sheet_name=0)
    st.write("현재 참외 데이터 확인", data)
    ...
    with st.form('Area-form'):
        ...
        if submitted:
            ...
            with st.spinner('모델 로딩 중입니다.'):
                m_neural_prophet = load("pages/smallmelon_neuralprophet_model.np") #모델 로드
                time.sleep(5)
            st.success('예측 완료!')
            days = 365
            ...
            week_weight = pd.read_excel('pages/적용할_참외_요일가중치.xlsx', sheet_name=0)
            ...
            end_surplus_list = []
            for i in range(6):
                if start_day_of_week == (6 - i):  # 일요일, 토요일, ... 시작인 경우
                    start_surplus_list = np_data[0:(i + 1), 1]
                    np_data = np_data[(i + 1):]
                    break
            for i in range(6):
                if end_day_of_week == (i):  # 월요일, 화요일, ... 마지막인 경우
            ...
            for i in range(len(start_surplus_list)):
                if len(start_surplus_list)>1:
                    if i == (len(start_surplus_list)-2):
                        start_surplus_list[i] = 0
            start_surplus_list = pd.DataFrame(start_surplus_list, columns=["가중치 적용 결과"])
            if len(end_surplus_list) == 6:
                end_surplus_list[len(end_surplus_list)-1] = 0
            ...
            forecast_value= forecast_value[['ds','origin-0']].rename(columns={'ds':'날짜','origin-0':'예측된 출하량'})

            model_scores = pd.read_excel('pages/smallmelon_neuralprophet_model_MAE_RMSE.xlsx', sheet_name=0)
            st.write("사용된 모델의 성능")
            ...
            st.write("예측된 데이터의 추세 그래프")
            fig_forecast = px.line(forecast_value, x='날짜', y='예측된 출하량')
...
with tab2:
    ...
    st.markdown("- 날짜 (ds)")
    st.markdown("- 경매물량 (y)")
    st.markdown("- 토요일 (sat_day)")
    st.markdown("- 기온 (temp)")
    st.markdown("- 최고기온 (max_Temp)")
    st.markdown("- 최저기온 (min_Temp)")
    st.markdown("- 습도 (hum)")
    st.markdown("- 풍량 (wind)")
    st.markdown("- 강수량 (rain)")
    st.markdown("- 일조시간 (sum_Time)")
    st.markdown("- 일사량 (sun_Qy)")
    st.markdown("- 결로시간 (condens_Time)")
    st.markdown("- 초상온도 (gr_Temp)")
    st.markdown("- 지중온도 (soil_Temp)")
    st.markdown("- 토양수분보정값 (soil_Wt)")
    st.markdown("")
    st.markdown("")
    st.markdown('#### :white_check_mark: 학습 데이터')
    old_data = pd.read_excel('pages/smallmelon_train_data.xlsx', sheet_name=0)
    ...
    with st.form('학습데이터 업로드'):
        ...
        if submitted: 
                ...
                with st.spinner('Neural Prophet 학습중입니다.'):
                    ...
                    test_data = dataframe.iloc[len(train_data):, :]
                    set_log_level("ERROR")
                    warnings.filterwarnings("ignore")
                    warnings.filterwarnings("ignore", category=DeprecationWarning)
                    set_random_seed(0)
                    col_lst = ['sat_day', 'temp', 'max_Temp', 'min_Temp', 'hum', 'wind', 'rain', 'sun_Time', 'sun_Qy', 'condens_Time', 'gr_Temp', 'soil_Temp', 'soil_Wt'] 
                    ...
                    data = np.array(forecast_value)
                    week_weight = pd.read_excel('pages/적용할_참외_요일가중치.xlsx', sheet_name=0)
                    week_weight = np.array(week_weight)
                    start_date = pd.to_datetime(data[0, 0])
                    end_date = pd.to_datetime(data[len(data)-1, 0])
                    start_day_of_week = start_date.weekday()
                    end_day_of_week = end_date.weekday()
                    start_surplus_list = []
                    end_surplus_list = []
                    for i in range(6):
                        if start_day_of_week == (6 - i):  
                            start_surplus_list = data[0:(i + 1), 1]
                            data = data[(i + 1):]
                            break  
                    for i in range(6):
                        if end_day_of_week == (i):  
                            end_surplus_list = data[-(i+1):, 1]
                            data = data[:-(i+1)]
                            break  
                    weekend_sum_list = []
                    sum = 0 
                    ...
                    df.to_excel('pages/smallmelon_neuralprophet_model_MAE_RMSE.xlsx', index=False) # 파일로 저장
                    ...
                    save(real_m_neuralprophet, "pages/smallmelon_neuralprophet_model.np")
                    time.sleep(5)
