...
tab1, tab2 = st.tabs(['오이 출하량 예측', '오이 모델 재학습'])
with tab1:
    st.header('오이 출하량 예측')
    data = pd.read_excel('pages/cucumber_train_data.xlsx', sheet_name=0)
    st.write("현재 오이 데이터 확인", data)
    ...
    with st.form('Area-form'):
        ...
        if submitted:
            ...
            with st.spinner('모델 로딩 중입니다.'):
                m_neural_prophet = load("pages/cucumber_neuralprophet_model.np") #모델 로드
                time.sleep(5)
            st.success('예측 완료!')
            days = 365
            ...
            week_weight = pd.read_excel('pages/적용할_오이_요일가중치.xlsx', sheet_name=0)
            ...
            for k in range(len(weekend_sum_list)):
                for w in range(7):  # 0에서 6까지, 7일 간의 가중치를 적용합니다.
            ...
            model_scores = pd.read_excel('pages/cucumber_neuralprophet_model_MAE_RMSE.xlsx', sheet_name=0)
            st.write("사용된 모델의 성능")
...
with tab2:
    ...
    old_data = pd.read_excel('pages/cucumber_train_data.xlsx', sheet_name=0)
    ...
    with st.form('학습데이터 업로드'):
        ...
        if submitted: # 확인이 눌렸을때        
                ...
                with st.spinner('Neural Prophet 학습중입니다.'):
                    ...
                    set_random_seed(0)
                    col_lst = ['sun_day', 'temp', 'max_Temp', 'min_Temp', 'wind', 'rain', 'condens_Time'] 
                    ...
                    forecast_value = forecast_value[['ds','origin-0']]
                    data = np.array(forecast_value) # 수정 데이터
                    week_weight = pd.read_excel('pages/적용할_오이_요일가중치.xlsx', sheet_name=0)
                    week_weight = np.array(week_weight)
                    ...
                    df.to_excel('pages/cucumber_neuralprophet_model_MAE_RMSE.xlsx', index=False) # 파일로 저장
                    ...
                    save(real_m_neuralprophet, "pages/cucumber_neuralprophet_model.np")
                    time.sleep(5)
